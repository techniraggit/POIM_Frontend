...
const Filters = ({
  ...
  useEffect(() => {
    if (
      query &&
      !Object.keys(query).some((data) => query[data] !== "") &&
      inputValue === ""
    ) {
      applyFilters({});
    } else {
    ...
    <div className="filter-wrapper">
      <div className="filter-main">
        <div className="wrapin-form add-clear-wrap mt-0 ps-3">
          ...
          {role && (
            <Select
              className="line-select dropdown-report me-2"
              placeholder="Role"
              onChange={(value) =>
                setQuery((prevState) => ({
                  ...prevState,
          ...
          {name && (
            ...
            >
              {poName.map((entry) => (
                <Select.Option
                  key={entry.created_by__id}
                  value={entry.created_by__id}
                >
                  {entry.full_name}
                </Select.Option>
              ))}
